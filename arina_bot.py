...
@MypyBot.message_handler(commands=['color'])
def button_pmenu(message):
    markup = types.ReplyKeyboardMarkup()
    item1=types.KeyboardButton("Blue")
    item2=types.KeyboardButton("Pink")
    item3=types.KeyboardButton("Red")
    markup.add(item1, item2, item3)
    MypyBot.send_message(message.chat.id,'Выберите цвет',reply_markup=markup)
...
def replyer(message):
    match message.text:
        case "Blue": 
            MypyBot.reply_to(message, "Прикольно,я  люблю этот цвет")
        case "Pink":
            MypyBot.reply_to(message, "ммм Вкусненько")
        case "Red":
            MypyBot.reply_to(message, "а эту я не люблю")
